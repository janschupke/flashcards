...
export const CharacterInfoColumn: React.FC<CharacterInfoColumnProps> = ({
  ...
  return (
    <div className="flex flex-col items-center flex-1">
      <div className="text-xs sm:text-sm text-text-tertiary mb-1 uppercase tracking-wider">{label}</div>
      <div className={`text-lg sm:text-xl md:text-2xl font-medium ${valueClassName}`}>{value}</div>
    </div>
  );
};
